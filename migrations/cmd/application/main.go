...
import (
	...
	"database/sql"
	"flag"
	"github.com/GermanBogatov/auth-service/migrations/configs"
	psqlMigration "github.com/GermanBogatov/auth-service/migrations/psqlmigrations"
...
func init() {
	systemName := os.Getenv("AUTH_MIGRATION_SERVICE_MIGRATIONS_SYSTEM_NAME")
	if systemName == "" {
		systemName = configs.Namespace
	}

	serviceEnv := os.Getenv("AUTH_MIGRATION_SERVICE_MIGRATIONS_ENV")
	if serviceEnv == "" {
		serviceEnv = "dev"
	}
	logLevel := os.Getenv("AUTH_MIGRATION_SERVICE_MIGRATIONS_LOG_LEVEL")
	if logLevel == "" {
		logLevel = "INFO"
	}
